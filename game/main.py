...
# - Fixed: ASCII text only (removed smart quotes/ellipsis to prevent font crashes)
# - Fixed: Added font safety checks
# ---------------------------------------------------------
...
def draw_text(surf, text, x, y, color=WHITE, font=None):
    # Safety: fallback to global FONT if none passed, or return if still None
    if font is None:
        font = FONT
    if font is None:
        return
    ...
    try:
        ...
        # Prevents crash if render fails on a specific character
        pass
...
def begin_game():
    ...
    # FIXED: Replaced '…' with '...' and smart quotes with normal ones
...
async def main():
    ...
    pygame.font.init()
    # Using None requests default system font, which is where unicode often fails
