...
# We initialize these to None, but will set them in main()
FONT = None
BIG = None
HUGE = None
...
def wrap_lines(text, max_width_px, font):
    if font is None: font = FONT
    if font is None: return [text]
    
    words = text.split()
    lines, cur = [], []
    for w in words:
        # Safety for weird font sizing
        try:
            w_w = font.size(w)[0]
        except:
            w_w = 10
...
class GameState:
    def __init__(self):
        ...
        self.flags = {
            "met_susan": False, "met_peter": False, "met_duncan": False,
            "met_teacher": False, "suspicious": 0, "found_clue": False,
            "learned_schedule": False, "got_help": False, "made_plan": False,
            "ready_finale": False,
    }
...
def draw_alien_teacher(screen, x, y, scale=1.0, with_pointer=True):
    ...
    # Suit
    suit_w, suit_h = int(40 * scale), int(44 * scale)
...
def draw_actor(screen, a: Actor):
    ...
    if a.name == "Mr. Smith":
        ...
        draw_prop(screen, x, y, a.radius, a.color)
...
def draw_dialog(screen):
    if not state.active_dialog: return
    box_w, box_h = WIDTH - 80, 220
    box_x, box_y = 40, HEIGHT - box_h - 30
...
def update_toast():
    if state.toast_timer > 0:
        state.toast_timer -= 1
        if state.toast_timer <= 0: state.toast = ""

# ---------------- Scenes ----------------

def build_scene(scene_name):
    print(f"DEBUG: Building Scene {scene_name}")
    ...
    if scene_name == "schoolyard":
        ...
        all_sprites.add(teacher); props.append(teacher)
...
def talk_to(name):
    if state.scene == "schoolyard":
        if name == "Susan Simmons":
            ...
            state.push_dialog(DialogChoice("Susan looks like she's already solving something. 'You feel it too, right?'", ["Ask what she noticed", "Agree and watch together", "Say she's overreacting"], after))
            state.next_dialog()
            ...
            state.push_dialog(DialogChoice("Peter grins nervously. 'This might be the most interesting day ever.'", ["Tell him to stay calm", "Ask what he saw", "Ask him to help investigate"], after))
            state.next_dialog()
            ...
            state.push_dialog(DialogChoice("Duncan sizes you up. 'What are YOU staring at?'", ["Tell him nothing", "Ask him to help (carefully)", "Ask if he noticed the teacher"], after))
            state.next_dialog()
        elif name == "School Door":
            if state.flags["met_susan"] and state.flags["met_peter"] and state.flags["met_duncan"]:
                state.push_dialog(DialogChoice("The school door swings open. The air inside smells like pencils... and secrets.", ["Go in", "Go in", "Go in"], lambda idx: (set_scene("classroom"), setattr(state, "objective", "In class: talk to Mr. Smith and check the Teacher Desk."))))
                state.next_dialog()
            else:
    ...
    if name == "Mr. Smith":
        def after(i):
            ...
            state.push_dialog(DialogChoice("Mr. Smith writes one word on the board: 'OBSERVE.' Then he turns too smoothly.", ["Ask if he's 'normal'", "Act like everything's fine", "Stay quiet and watch"], after))
            state.next_dialog()
            ...
            if not state.flags["found_clue"]:
                ...
                state.push_dialog(DialogChoice("The teacher's desk drawer sticks for a second... then opens.", ["Search carefully for anything odd", "Listen first, then peek", "Close it - too risky"], after))
                state.next_dialog()
            else:
                state.set_toast("You already searched the desk.", 160)
        elif name == "Hallway Door":
            state.push_dialog(DialogChoice("The hallway feels louder than it should be. Like the building is whispering.", ["Go out", "Go out", "Go out"], lambda idx: (set_scene("hallway"), setattr(state, "objective", "Check the Notice Board and decide your next move."))))
            state.next_dialog()
    ...
    if name == "Notice Board":
        def after(i):
            if i == 0:
                ...
                if state.flags["found_clue"] and state.flags["learned_schedule"]: state.objective = "Go to the Storage Door to regroup."
                else: state.objective = "You need more: find a clue AND learn what the symbols mean."
            state.push_dialog(DialogChoice("The notice board is covered in papers. One page has markings that don't look like school stuff.", ["Study the weird page", "Wave Susan over", "Back away"], after))
            state.next_dialog()
        elif name == "Storage Door":
            if state.flags["found_clue"] and state.flags["learned_schedule"]:
                state.push_dialog(DialogChoice("The storage door is usually locked... but today it opens. Like it was waiting.", ["Go in", "Go in", "Go in"], lambda idx: (set_scene("plan_room"), setattr(state, "objective", "Make a plan together (Idea Board)."))))
                state.next_dialog()
            else:
                state.set_toast("You're not ready. You need more evidence first.", 220)
    elif name == "Back to Class":
        state.push_dialog(DialogChoice("Go back to class?", ["Yes", "Yes", "Yes"], lambda idx: (set_scene("classroom"), setattr(state, "objective", "Find a clue (Teacher Desk) and return."))))
        state.next_dialog()
    ...
    if name == "Idea Board":
        def after(i):
            ...
            state.push_dialog(DialogChoice("You huddle up. Susan wants proof. Peter wants bold action. Duncan wants it over with.", ["Susan plan: controlled distraction", "Peter plan: public confrontation", "Duncan plan: bait-and-reveal"], after))
            state.next_dialog()
        elif name == "Auditorium Door":
            if state.flags["ready_finale"]:
                state.push_dialog(DialogChoice("The auditorium is empty... but it feels like a stage waiting for one moment.", ["Enter", "Enter", "Enter"], lambda idx: (set_scene("finale"), setattr(state, "objective", "Trigger Stage Control, then go to Big Reveal Spot."))))
                state.next_dialog()
            else:
                state.set_toast("You're not ready yet. Make a plan first.", 200)
        elif name == "Back Hall":
            state.push_dialog(DialogChoice("Go back to the hallway?", ["Yes", "Yes", "Yes"], lambda idx: (set_scene("hallway"), setattr(state, "objective", "Finish gathering info, then regroup at Storage Door."))))
            state.next_dialog()
    ...
    if name == "Stage Control":
        def after(i):
            bump_suspicion()
            state.set_toast("Something shifts... like a disguise doesn't fit anymore.", 240)
            state.push_dialog(DialogChoice("A stage control panel. One switch labeled: 'AUDIO / LIGHTS.'", ["Flip it now", "Wait for Mr. Smith to approach", "Signal your friends first"], after))
            state.next_dialog()
        elif name == "Big Reveal Spot":
            if state.flags["suspicious"] >= 3 and (state.flags["made_plan"] or state.flags["ready_finale"]):
                def after(i):
                    if i == 0: ending("REVEAL")
                    elif i == 1: ending("SAVE")
                    else: ending("BOLD")
                state.push_dialog(DialogChoice("This is it. If you're right, the truth comes out. If you're wrong... awkward.", ["Reveal him publicly", "Protect your friends first", "Confront him directly"], after))
                state.next_dialog()
            else:
                state.set_toast(f"Not ready: suspicion={state.flags['suspicious']}, made_plan={state.flags['made_plan']}, ready_finale={state.flags['ready_finale']}", 260)
        elif name == "Mr. Smith":
            def after(i):
                bump_suspicion()
                state.set_toast("Mr. Smith: 'Observation is... instructive.'", 220)
            state.push_dialog(DialogChoice("Mr. Smith stands near the stage, too calm for an empty auditorium.", ["Ask what he is", "Ask what he wants", "Say nothing and watch"], after))
            state.next_dialog()

def ending(kind):
    if kind == "REVEAL": msg = "You force the moment into the open. Under bright lights, the 'normal' act slips - and something alien flashes through.\n\nPeople gasp. Adults freeze. But you did it: the truth is public.\nWhatever happens next... you changed the story."
    elif kind == "SAVE": msg = "You choose people over proof. You pull your friends away first, even if the truth is messier.\n\nYou don't get a perfect 'gotcha' moment... but you keep everyone together."
    else: msg = "You step forward and confront him. It's terrifying - but honest.\n\nFor one heartbeat, the 'teacher' looks almost impressed.\nCourage is a kind of intelligence."
    state.push_dialog(DialogChoice(msg + "\n\n(THE END)  Press 1 to restart, or ESC to quit.", ["Restart", "Restart", "Restart"], lambda idx: restart_game()))
    state.next_dialog()
...
def draw_background(screen):
    if state.scene == "schoolyard": screen.fill((25, 50, 40))
    elif state.scene == "classroom": screen.fill((30, 45, 55))
    elif state.scene == "hallway": screen.fill((45, 35, 35))
    elif state.scene == "plan_room": screen.fill((30, 30, 45))
    else: screen.fill((35, 25, 40))
...
def draw_title_screen(screen):
    ...
    draw_text(screen, "ENTER = Start    |    ESC = Quit", cx - FONT.size("ENTER = Start    |    ESC = Quit")[0] // 2, HEIGHT - 120, (230, 210, 120), FONT)
...
async def main():
    ...
    try:
        ...
        HUGE = pygame.font.SysFont("monospace", 64, bold=True)
    except:
        print("Font Init Failed, retrying default")
...
for a in all_sprites:
    if isinstance(a, Actor): a.update()
