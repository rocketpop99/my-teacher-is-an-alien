...
# ---------------------------------------------------------
# My Teacher Is an Alien (inspired) - Choice Game
# - No enemies / no shooting
# - Move + interact + text choices
# - Simple shapes + multiple sprites
# - Pygbag-friendly async loop
# ---------------------------------------------------------
...
def make_tone(freq_hz=440, ms=150, volume=0.25, sample_rate=44100, wave="sine"):
    """Generate a simple mono tone as pygame.mixer.Sound (no external files)."""
...
def make_room_loop(freq=220, volume=0.12):
    # A longer tone loop (soft) to act like “room music”
...
# ---------------- Drawing: people + alien teacher ----------------

def draw_little_person(screen, x, y, body_color, scale=1.0):
    # Simple little person (head + body + legs)
    head_r = int(8 * scale)
    body_w = int(14 * scale)
    body_h = int(18 * scale)
...
def draw_alien_teacher(screen, x, y, scale=1.0, with_pointer=True):
    # Alien teacher: big head, antenna, suit, tie, "teacher pointer"
    head_w = int(34 * scale)
    head_h = int(46 * scale)
...
def draw_actor(screen, a: Actor):
    ...
    if a.name == "Mr. Smith":
        ...
        # player is blue “person”, NPCs are colored “people”
        body_color = a.color
                                           ...
                                           def after(i):
                                               ...
                                               state.set_toast("Something shifts… like a disguise doesn’t fit anymore.", 240)
                                                   ...
                                                   state.next_dialog()
                                                           ...
                                                           # FIX: allow reveal if you have suspicion AND you reached finale route.
                                                           # (players can reach finale but skip Idea Board interaction)
                                                                               ...
                                                                               ending("SAVE")
                                                                                       ...
                                                                                       ending("BOLD")
                                                                                           ...
                                                                                           state.next_dialog()
                                                                                                   ...
                                                                                                   state.set_toast(
                                                                                                                   f"Not ready: suspicion={state.flags['suspicious']}, made_plan={state.flags['made_plan']}, ready_finale={state.flags['ready_finale']}",
                                                                                                                   260
                                                                                                                   )
